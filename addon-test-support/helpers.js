...
export async function selectChooseIndex(path, index) {
  ...
  if (!trigger) {
    throw new Error(`You called "selectChoose('${path}', '${index}')" but no select was found using selector "${path}"`);
  }
  ...
  let option = document.querySelector(`.${optionsClass} .ui-select-option:nth-child(${index})`);
...
export async function selectChoose(path, value) {
  ...
  if (!trigger) {
    throw new Error(`You called "selectChoose('${path}', '${value}')" but no select was found using selector "${path}"`);
  }
  ...
  let option = document.querySelector(`.${optionsClass} [data-value='${value}']`);
